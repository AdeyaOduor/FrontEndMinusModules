...
function MonitorTreeStatus({ onSave, onCloseForm }) {
  ...
  const handleStatusSubmit = async () => {
    ...
    const response = await fetch("http://localhost:8080/api/v1/treeMonitoring/create?plantingSessionId=1", {
      method: "POST",
      body: formData,
    });
      ...
      <FormContainer style={{ background: "#e0e0e0", padding: "20px" }}>
        <form>
          ...
          <TextField
            ...
            onChange={(e) => setNumWoodlotTreesAlive(Math.max(0, e.target.value))}
            required
          />
          <TextField
...
<Grid item xs={12} sm={6} lg={6} style={{ display: 'flex', flexDirection: "row", padding:'1px', marginTop:'20px' }}>
            {/* <Grid > */}
...
function SCDEMonitoringPage() {
  ...
  const fetchData = async () => {
    const response = await fetch("http://localhost:8080/api/v1/treeMonitoring/read?plantingSessionId=1");
    if (response.ok) {
        ...
        >
         ...
         {isFormOpen && (
          <Dialog open={isFormOpen} onClose={() => setIsFormOpen(false)}>
            <DialogTitle style={{color:'orange', textAlign:'center' }}>Add Monitoring Data</DialogTitle>
            <DialogContent>
              <MonitorTreeStatus onSave={handleFormSubmit} onCloseForm={() => setIsFormOpen(false)} />
            </DialogContent>
          </Dialog>
        )}
      </div>
...
export default SCDEMonitoringPage;

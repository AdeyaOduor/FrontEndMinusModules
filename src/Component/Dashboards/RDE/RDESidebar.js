...
const items = [
  ...
  {
    href: '/RDE/rdetargettrees',
    icon: EventAvailableIcon,
    title: 'Targets Trees'
  },
  {
    href: '/RDE/rdetreeplanting',
    icon: PublicIcon,
    title: 'TreeData Planting'
  },
  {
    href: '/RDE/rdemonitortreestatus',
    icon: InsightsIcon,
    title: 'Tree Monitoring'
  },
  {
    href: '/RDE/schoolreports',
    icon: SchoolIcon,
...
const DashboardSidebar = ({ onMobileClose, openMobile }) => {
  ...
  const handleLogout = () => {
    ...
    // For example, clear the user's session and redirect to the login page
  };
  ...
  const content = (
    ...
    >
      ...
      >
        <Avatar src='https://cdn-icons-png.flaticon.com/128/149/149071.png' alt='Avatar'
        ...
        >
          {getInitials(user?.fullNames || '')}
        </Avatar>
        <Typography color="textPrimary" variant="h5">
          {user?.fullNames || 'Guest'}
        </Typography>
        <Typography color="textSecondary" variant="body2">
          {user?.authorities || 'RDE'}
        </Typography>
      </Box>
      <Divider />
      <Box sx={{ p: 2 }}>

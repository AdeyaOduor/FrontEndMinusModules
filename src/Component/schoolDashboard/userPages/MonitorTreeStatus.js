...
function MonitorTreeStatus({ onSave, onCloseForm }) {
  ...
  const handleStatusSubmit = async () => {
    ...
    axios.post("http://localhost:8080/api/v1/treeMonitoring/create?plantingSessionId=1", formData)
...
function MonitoringPage() {
  ...
  const fetchData = () => {
    axios
      .get("http://localhost:8080/api/v1/treeMonitoring/read?plantingSessionId=1")
      .then(function (response) {
        if (response.status === 200) {
          response.data.then(function (monitoringData) {
            setData(monitoringData);
          });
        } else {
         
          console.error("Non-OK response status:", response.status);
          // Optionally, you can perform additional error handling or show an error message to the user.
        }
      })
      .catch(function (error) {
